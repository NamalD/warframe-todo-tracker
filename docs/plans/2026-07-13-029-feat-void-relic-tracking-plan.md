...
**Blocker resolved (2026-07-16):** Confirmed `@wfcd/items` relic objects expose `vaulted` (boolean) and `rewards[].rarity` (string: "Common"/"Uncommon"/"Rare"). Relic tier/era (Axi/Meso/Neo/Lith/Requiem) is embedded in the `name` field, not a separate property. Requiem relics have 8 rewards (all Rare) instead of 6.
