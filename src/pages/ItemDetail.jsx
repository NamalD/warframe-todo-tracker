...
function ItemDetail() {
  ...
  const trackToggle = () => {
    // In-memory toggle of seed data for demo purposes
          ...
          <tbody>
            {materials.map((m) => (
              <tr key={m.id}>
                ...
                <td>
                  <Link to={`/sources`} state={{ material: m.material_name }}>
                    View sources
                  </Link>
                </td>
              </tr>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            {tree.parents.map((rel) => (
              <div key={rel.id} className="tree-row">
                <Link to={`/items/${rel.parent?.id}`}>{rel.parent?.name || 'Unknown'}</Link>{' '}
        ...
        {tree.children.length > 0 && (
          <div>
            ...
            {tree.children.map((rel) => (
              <div key={rel.id} className="tree-row">
                <Link to={`/items/${rel.child?.id}`}>{rel.child?.name || 'Unknown'}</Link>{' '}
                <span className="muted">x{rel.quantity_required}</span>
              </div>
            ))}
          </div>
